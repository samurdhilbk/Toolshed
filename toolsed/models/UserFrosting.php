...
/**
 * The ToolsEd application class, which extends the basic Slim application.  
 *
 * @package ToolsEd
 ...
 * @property \ToolsEd\SiteSettings $site
 * @property \ToolsEd\User $user
 * @property \ToolsEd\PageSchema $schema
 * @property \ToolsEd\JqueryValidationAdapter $jsValidator
 */
class ToolsEd extends \Slim\Slim {
    ...
    /**
     * Sets up key ToolsEd services: message stream, translator, and client-side validation adapter
     */    
    public function setupServices($locale){
        ...
        if (!isset($_SESSION['toolsed']['alerts']))
            $_SESSION['toolsed']['alerts'] = new \Fortress\MessageStream();
        
        $this->alerts = $_SESSION['toolsed']['alerts'];
     ...
     *
     * ToolsEd uses Slim's custom error handler to log the error trace in the PHP error log, and then generates a client-side alert (SERVER_ERROR).
    ...
    public function setupTwig(){
        ...
        /* Import ToolsEd variables as global Twig variables */    
        $twig = $this->view()->getEnvironment();   
    ...
    public function login($user, $remember = false){
        ...
        // Assume identity
        $_SESSION["toolsed"]["user_id"] = $user->id;
