 ...
 * @property string version The current version of ToolsEd.
 * @property string author The author of the site.  Will be used in the site's author meta tag.  
 */
class SiteSettings extends TEModel {    
    /**
    ...
    /**
     * @var array A list of plugin names => arrays of settings for that plugin.  The core plugin is "toolsed".
     */    
    protected $_settings;
    
    /**
     * @var array A list of plugin names => arrays of descriptions for that plugin.  The core plugin is "toolsed".
     */ 
     ...
     *
     * @return boolean true if the table exists and all keys (core toolsed and plugins) are defined in the table, false otherwise.
     */
    ...
    /**
     * Magic isset to determine if a particular setting is defined in the environment or core toolsed settings.
    ...
    public function __isset($name) {
        if (isset($this->_environment[$name]) || isset($this->_settings['toolsed'][$name]))
            return true;
        else
            return false;
    }
    ...
    /**
     * Magic setter to set the value of a core toolsed setting.
    ...
    public function __set($name, $value){
        return $this->set('toolsed', $name, $value);   
    }

    /**
     * Magic getter to get the value of an environment or core toolsed setting.  This does not get plugin settings.  For that, you must use `get`.
    ...
    public function __get($name){
        ...
        } else if (isset($this->_settings['toolsed'][$name])){
            return $this->_settings['toolsed'][$name];
        } else {
            throw new \Exception("The value '$name' does not exist in the core toolsed settings.");
        }
    }
     ...
     *
     * @param string $name The name of the setting.
     * @param string $plugin The plugin scope of this setting.  Defaults to "toolsed".
     * @throws Exception The value does not exist for this plugin.     
     */  
    public function get($name, $plugin = "toolsed"){
     ...
     */ 
    public function getDescription($name, $plugin = "toolsed"){
     ...
     * For example, $settings['toolsed']['site_title']['value'] = "ToolsEd".
    ...
    /**
     * Get an array of available locales for ToolsEd by scanning the path specified in $app->config('locales.path').
     *
    ...
    /**
     * Get an array of available themes for ToolsEd by scanning the path specified in $app->config('themes.path').
     *
