 ...
 * @see http://www.toolsed.com/tutorials/lesson-3-data-model/
 *
 * @property int id
...
class User extends TEModel {
    ...
    protected $_groups;
    /**
     * @var int[] An array of circle_ids to which this user belongs. An empty array means that the user's circles have not been loaded yet.
     */
    protected $_circles;
    ...
    public function events(){
        return $this->hasMany('ToolsEd\UserEvent');
    }
    ...
    public function groups(){
        ...
        return $this->belongsToMany('ToolsEd\Group', $link_table);
    }
    ...
    private function syncCachedGroups(){
        if (isset($this->_groups)) {
            $link_table = Database::getSchemaTable('group_user')->name;
            return $this->belongsToMany('ToolsEd\Group', $link_table)->sync($this->_groups);
        } else
            return false;
    }
    ...
    public function circles(){
        ...
        return $this->belongsToMany('ToolsEd\Circle', $link_table);
    }

    public function projects(){
            
        $link_table = Database::getSchemaTable('project_user')->name;
        return $this->belongsToMany('ToolsEd\Project', $link_table);
    }


    private function syncCachedCircles(){
        if (isset($this->_circles)) {
            $link_table = Database::getSchemaTable('circle_user')->name;
            return $this->belongsToMany('ToolsEd\Circle', $link_table)->sync($this->_circles);
        } else
            return false;
    }

    /**
     * Get the circles to which this User currently belongs, as currently represented in this object.
     *
     * This method caches the data after the first time loading from the database.  To force a refresh, use the `fresh` method.
     * @return array[Group] An array of Group objects, indexed by circle_id, to which this User belongs.
     */
    public function getCircles(){
        $this->getGroupIds();
        
        // Return the array of circle objects
        $result = Circle::find($this->_circles);
        
        $circles = [];
        foreach ($result as $cicle){
    ...
    /**
     * Get an array of circle_ids to which this User currently belongs, as currently represented in this object.
     *
     * This method does NOT modify the database.
     * @return array[int] An array of circle_ids to which this User belongs.
     */    
    public function getCircleIds(){
        // Fetch from database, if not set
        if (!isset($this->_circles)){    
            $link_table = Database::getSchemaTable('circle_user')->name;
            $result = Capsule::table($link_table)->select("circle_id")->where("user_id", $this->id)->get();
            
            $this->_circles = [];
            foreach ($result as $circle){
                $this->_circles[] = $circle['circle_id'];
            }      
        }
    ...
    /**
     * Add a circle_id to the list of _circles to which this User belongs, checking that the Group exists and the User isn't already a member.
     *     
     * This method does NOT modify the database.  Call `store` to persist to database. 
     * @param int $circle_id The id of the circle to add the user to.
     * @throws Exception The specified circle does not exist.
     * @return User this User object.
     */  
    public function addCircle($circle_id){
        $this->getCircleIds();
        
        // Return if user already in circle
        if (in_array($circle_id, $this->_circles))
            return $this;
        
        // Next, check that the requested circle actually exists
        if (!Circle::find($circle_id))
            throw new \Exception("The specified circle_id ($circle_id) does not exist.");
                
        // Ok, add to the list of circles
        $this->_circles[] = $circle_id;
        
        return $this;        
    }
    
    /**
     * Remove a circle_id from the list of _circles to which this User belongs, checking that the User is already a member.
     *
     * This method does NOT modify the database. Call `store` to persist to database.
     * @param int $circle_id The id of the circle to remove the user from.
     * @return User this User object.
     */   
    public function removeCircle($circle_id){
        // Fetch from database, if not set
        $this->getCircleIds();
        
        // Check that user not in circle     
        if (($key = array_search($circle_id, $this->_circles)) !== false) {
            unset($this->_circles[$key]);
        }
    ...
    private function fetchPrimaryGroup() {
        ...
        return $this->belongsTo('ToolsEd\Group', 'primary_group_id')->getEager()->first();
    }
     ...
     *
     * You assign this user's id to $_SESSION["toolsed"]["user_id"] after calling login, so that it will persist in the session.
     */
