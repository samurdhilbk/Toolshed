 ...
 */
class UserController extends \ToolsEd\BaseController {
    ...
    public function pageUserBio($id){

        $bio=UserBio::where('user_id',$id)->first();
        $bio_user=User::where('id',$id)->first();


        if(isset($bio_user['id'])){
            $this->_app->render('bio.twig', ['bio'=>$bio,'bio_user'=>$bio_user]); 
            return true;
        }   
        return false;
    }

    public function updateUserBio($id,$column,$value){

        $rec = UserBio::where('user_id', $id)->update([$column => $value]);

    }

    public function updateUserPhoto($id,$photo,$loc){
        ...
        if(isset($photo)){
          ...
          if(empty($errors)==true){
             $location=$loc.'.'.$file_ext;
             //error_log($location);
             move_uploaded_file($file_tmp,$location);
             UserBio::where('user_id', $id)->update(['photo' => $id.'.'.$file_ext]);
          }else{
             print_r($errors);
          }
       }
    ...
    public function getUserBio($id,$column){

        return UserBio::where('user_id', $id)->pluck($column);

    }

    public function getPublicUserList(){
        ...
        foreach($users as $user){
            $ret[$user->id]=array("label"=>($user->user_name.' '.$user->display_name),"photo"=>(UserBio::where('user_id',$user->id)->pluck('photo')));
        }
    ...
    public function getUserCircleList($user_id){


        $circles=User::find($user_id)->circles;
    ...
    public function getUserProjectList($user_id){
