 ...
 *
 * Controller class for /install/* URLs.  Handles activities for installing ToolsEd.  Not needed after installation is complete.
 *
 * @package ToolsEd
 * @author Samurdhi Karunarathne
 * @link http://www.toolsed.com/navigating/#structure
 */
class InstallController extends \ToolsEd\BaseController {

    /**
     * Renders the initial page that comes up when you first install ToolsEd.
     *
     ...
     * 3. Check that we can connect to the database, as configured in `config-toolsed.php`.
    ...
    public function pageSetupDB(){
        ...
        if (PHP_VERSION_ID < 50400){
            $messages[] = [
                ...
                "message" => "I'm sorry, ToolsEd relies on numerous features of PHP that are only available in PHP 5.4 or later.  Please upgrade your version of PHP, or contact your web hosting service and ask them to upgrade it for you."
            ];
        }
        ...
        if (!class_exists('PDO')){
            $messages[] = [
                ...
                "message" => "I'm sorry, you must have PDO installed and enabled in order for ToolsEd to access the database.  If you don't know what PDO is, please see <a href='http://php.net/manual/en/book.pdo.php'>http://php.net/manual/en/book.pdo.php</a>.  You must also have MySQL version 4.1 or higher installed, since ToolsEd relies on native prepared statements."
            ];
        }
        ...
        if (!Database::testConnection()){
            $messages[] = [
                ...
                "message" => "Make sure that your database is properly configured in <code>config-toolsed.php</code>, and that you have selected the correct configuration mode ('dev' or 'production').  Also, make sure that your database user has the proper privileges to connect to the database."
            ];
        }
        ...
        if (count($tables) > 0){
            $messages[] = [
                ...
                "message" => "The following tables already exist in the database: <strong>" . implode(", ", $tables) . "</strong>.  Do you already have another installation of ToolsEd in this database?  Please either create a new database (recommended), or change the table prefix in <code>config-toolsed.php</code> if you cannot create a new database."
            ];
        }
        ...
        error_log("Installing");
            ...
            $messages[] = [
                ...
                "message" => "Hooray!  We were able to connect to your database and create the core tables for ToolsEd.",
                "class" => "success"
            ];
            if (PHP_VERSION_ID >= 50400 && PHP_VERSION_ID < 50500){
               $messages[] = [
                    "title" => "<i class='fa fa-warning'></i> PHP version",
                    "message" => "You currently have version " . PHP_VERSION . " of PHP installed.  We recommend version 5.5 or later.  ToolsEd can still be installed, but we highly recommend you upgrade soon.",
                    "class" => "warning"
                ];
            } else {
                $messages[] = [
            ...
            if (!(extension_loaded('gd') && function_exists('gd_info'))) {
                $messages[] = [
                    ...
                    "message" => "We could not confirm that the <code>GD</code> library is installed and enabled.  GD is an image processing library that ToolsEd uses to generate captcha codes for user account registration.  If you don't need captcha, you can disable it in Site Settings and ignore this message.  Otherwise, please see the <a href='http://www.toolsed.com/troubleshooting/' target='_blank'>troubleshooting guide</a> for information on installing and configuring GD.",
                    "class" => "warning"
                ];
            } else {
                if (!function_exists('imagepng')){
                    $messages[] = [
                        ...
                        "message" => "The <code>GD</code> library is installed and enabled, but PNG functions do not seem to be available.  ToolsEd uses the PNG functions of GD, an image processing library, to generate captcha codes for user account registration.  If you don't need captcha, you can disable it in Site Settings and ignore this message.  Otherwise, please see the <a href='http://www.toolsed.com/troubleshooting/' target='_blank'>troubleshooting guide</a> for information on updating GD to support PNG operations.",
                        "class" => "warning"
                    ];
                }
            }
